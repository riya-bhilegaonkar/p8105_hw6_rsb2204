...
model, we use the stepwise approach which comapres successive models and
identifies the best subsets. For my model I used backward elimination.
For this I first started with all the predictios in the model and then
...
model and remvoe the next least signficant predictor. The end results of
the `step()`, the summary for the anova, shows which variables need to
