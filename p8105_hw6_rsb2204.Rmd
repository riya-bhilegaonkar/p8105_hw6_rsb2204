...
In order to chose the variables for our model, we use the stepwise approach which comapres successive models and identifies the best subsets. For my model I used backward elimination. For this I first started with all the predictios in the model and then removed the predictors with the highest p-value (p-value) (those which are not significant <0.05). The `step()` with the direction of 'backward' function performs this analysis as it continues to re-fit the model and remvoe the next least signficant predictor. The end results of the `step()`, the summary for the anova, shows which variables need to be removed from the model. 
